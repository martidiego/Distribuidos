...
defp masterAux(workers, num1, listaPares, c_pid) do
	...
	if sumaB == num1 and sumaA != num1 and not(Enum.member?(listaPares,{sumaA,num1})), do: masterAux(workers, num1+1, listaPares++[{num1,sumaA}], c_pid), else: masterAux(workers, num1+1, listaPares, c_pid)
end


recibir_mensajes (timeout,worker_div, worker_suma, worker_sdp, c_pid, retry, idOperaciones, num, yaMandado) do
	receive do
					...
					sum = receive do
						{:replySuma, suma, idOp, worker_suma} -> 	
							if idOp == idOperaciones do
								...
								send(worker_div, {:reqWorkerDiv, {self(), num, idOperaciones}})	
								recibir_mensajes(timeout,worker_div, worker_suma, worker_sdp,c_pid,retry,idOperaciones,num,0)
							end
						...
						{:replySDP, sumDivisoresProp, idOp, worker_sdp} -> 		
							if idOp == idOperaciones do
							 	...
							 	action(timeout,worker_div, worker_suma, worker_sdp,c_pid,retry,idOperaciones,num,0)
							end
						...
						sum			#ESTOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO?????????????????????????????????????
				else
					#Descarta respuesta recibida
					send(worker_div, {:reqWorkerDiv, {self(), num, idOperaciones}})	
					recibir_mensajes(timeout,worker_div, worker_suma, worker_sdp,c_pid,retry,idOperaciones,num,1)
				end
		...
		{:replySDP, sumDivisoresProp, idOp, worker_sdp} -> 		
			if idOp == idOperaciones do
				...
				send(worker_sdp, {:reqWorkerDiv, {self(), num, idOperaciones}})	
			 	recibir_mensajes(timeout,worker_div, worker_suma, worker_sdp,c_pid,retry,idOperaciones,num,1)
			end
...
def action(timeout,worker_div, worker_suma, worker_sdp, c_pid, retry, idOperaciones, num, yaMandado) when retry < 5 do
	...
	recibir_mensajes(timeout,worker_div, worker_suma, worker_sdp, c_pid, retry, idOperaciones, num, yaMandado)
	end
